...
| Asked for 11 GB, got 22 GB | Connector **2.0.9+** (fix in 2.0.8) uses `+delta G` after reading real disk size; re-import **2.0.9** if UI still shows 2.0.8 |
...
| 2.0.9+ | Same as 2.0.8; bump version so FortiSOAR replaces code on upgrade (re-import or pack date refresh) |
